...
$(document).ready(function() {
	...
	manageCreditTable = $("#managecreditTable").DataTable({
		'ajax': 'php_action/fetchcredit.php',
		'order': []		
	});
$("#date").datepicker();
		...
		$.ajax({
			...
			success:function(response) {
				...
				$('#editcreditForm').unbind('submit').bind('submit', function() {
					...
					if(credit_type == "") {
						$("#credit_type").after('<p class="text-danger"> field is required</p>');

						$('#credit_type').closest('.form-group').addClass('has-error');
					} else {
